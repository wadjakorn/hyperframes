...
| [references/karaoke.md](references/karaoke.md)                           | Word-by-word active highlight (opt-in, loud/social) — per-token not a wipe, timing sources, HUD-safe placement.                    |
...
- **Transcription = WhisperX via `uvx`** (word-level timings + alignment; no manual install — `transcribe.cjs` drives `uvx whisperx`). Falls back to an existing word-level `transcript.json` if present. When WhisperX fails, the fallback to whisper.cpp is **not silent**: `transcribe.cjs` classifies the cause (`uv-missing` / `torchcodec-ffmpeg` / `hf-download-hang`) and prints the fix, warns up front on an ffmpeg-major mismatch (whisperx 3.8.6 torchcodec wants ffmpeg 4–7), and caps `HF_HUB_DOWNLOAD_TIMEOUT` so an IPv6-blackholed model CDN fails fast instead of hanging the run. Pure helpers are unit-tested in `transcribe.test.mjs`.
- **Per-project glossary + term-fix map** — ASR mis-hears domain terms (`Claude Code`, `tmux`, `Herdr`, `SSH`) and, in Thai, produces typos. `transcribe.cjs` reads a persisted **`captions.subs.json`** in the project (a stub is written on first run): `hotwords` are fed to WhisperX as an `--initial_prompt` to bias spelling, and `substitutions` (`{from,to}` literal + case-insensitive, or `regex: true`) are applied per word post-ASR (so they fix the whisper.cpp fallback path too). Edit it once and re-transcribe — no rebuilding the map every session.
